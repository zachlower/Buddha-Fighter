...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    /// <summary>
    /// Layer mask of walls that cannot be clung to.
    /// </summary>
    public LayerMask nonStickLayerMask;
    ...
    /// </summary>
    public float timeToGroundSpeed = 0.3f;
    ...
    ///<summary>
    /// clip to be played when grabbing wall
    /// </summary>
    ...
    /// Is slashing allowed?
    /// </summary>
    public bool enableSlashes = true;
    ...
    /// <summary>
    /// How far the motor will slash
    /// </summary>
    public float slashDistance = 0.4f;

    /// <summary>
    /// How long the slash lasts in seconds
    /// </summary>
    ...
    /// The easing function used during the dash. Pick 'Linear' for just a set speed.
    /// </summary>
    public EasingFunctions.Functions slashEasingFunction = EasingFunctions.Functions.EaseOutQuad;

    /// <summary>
    /// delay (in seconds) before gravity is turned back on after a slash.
    /// </summary>
    public float endSlashNoGravityDuration = 0.1f;

    /// <summary>
    /// delegate to attach when the motor slashes
    /// </summary>
    public Action onSlash;

    /// <summary>
    /// delegate to attach to when the motor's slash ends.
    /// </summary>
    public Action onSlashEnd;

    /// <summary>
    /// effect to display when word is impacted
    /// </summary>
    public GameObject wordImpact;
    ...
    public Vector2 velocity
    {
        get
        {
            ...
            {
                return _slashing.slashDir * GetSlashSpeed();
            }else
            {
                return _velocity;
            }
    ...
    /// <summary>
    /// returns the direction of the current slash. if not slashing then returns vector2.zero
    /// </summary>
    public Vector2 slashDirection
    {
        get
        {
            return IsSlashing() ? _slashing.slashDir : Vector2.zero;
        }
    }

    /// <summary>
    /// Returns the amount of distance slashed. If not slashing then returns 0.
    /// </summary>
    public float distanceSlashed
    {
        get
        {
            return IsSlashing() ? _slashing.distanceSlashed : 0;
        }
    }

    /// <summary>
    /// This is the distance calculated for slashed. Not be confused with distanceSlashed. This doesn't care if the motor has
    /// hit a wall.
    /// </summary>
    public float slashDistanceCalculated
    {
        get
        {
            return IsSlashing() ? _slashing.distanceCalculated : 0;
        }
    }

    /// <summary>
    /// If the motor is currently able to slash.
    /// </summary>
    public bool canSlash
    {
        get { return _slashing.enabled; }
        //TODO: should this also check if currently dashing? vice versa for canDash
    }
    ...
    /// <summary>
    /// Called when the player is trying to drop through the platform on which they are currently standing.
    /// </summary>
    public void Drop()
    {
        if(IsGrounded())
    ...
    /// <summary>
    /// Turn the platform's collider off for a brief period of time, then re-enable it.
    /// </summary>
    ...
    /// <summary>
    /// Call this to have the motor try to slash, once called it will be handled in the FixedUpdate tick.
    /// This causes the object to attack quickly
    /// </summary>
    public void Slash()
    {
        _slashing.pressed = true;
        _slashing.slashWithDirection = false;
    }

    /// <summary>
    /// Forces the motor to slash regardless if the motor thinks it is valid or not.
    /// </summary>
    public void ForceSlash()
    {
        Slash();
        _slashing.force = true;
    }

    /// <summary>
    /// Send a direction vector to slash allow slashing in a specific direction.
    /// </summary>
    /// <param name="dir">The normalized direction of the slash.</param>
    public void Slash(Vector2 dir)
    {
        _slashing.pressed = true;
        _slashing.slashWithDirection = true;
        _slashing.slashDir = dir;
    }

    /// <summary>
    /// Forces a slash along a specified direction.
    /// </summary>
    /// <param name="dir">The normalized direction of the slash.</param>
    public void ForceSlash(Vector2 dir)
    {
        Slash(dir);
        _slashing.force = true;
    }
    ...
    /// <summary>
    /// Call to end slash immediately.
    /// </summary>
    public void EndSlash()
    {
        // If slashing then end now.
        if (IsSlashing())
        {
            _slashing.pressed = false;
            if (_hitWords)
            {
                _velocity = _slashing.slashDir * GetSlashSpeed();
                _slashing.gravityEnabledFrames = GetFrameCount(endSlashNoGravityDuration);
            }else
            {
                //_velocity.x = _slashing.slashDir.x * GetSlashSpeed();
                _slashing.gravityEnabledFrames = -1;
            }
    ...
    /// <summary>
    /// Is the motor Slashing? 
    /// </summary>
    public bool IsSlashing()
    {
        return motorState == MotorState.Slashing;
    }
    ...
    private EasingFunctions.EasingFunc _slashFunction;
    private EasingFunctions.EasingFunc _slashDerivativeFunction;
    ...
    private EasingFunctions.Functions _currentSlashEasingFunction;
    ...
    // contains the various slash variables
    private class SlashState
    {
        public bool pressed;
        public bool enabled = true;
        public int slashingFrames;
        public bool slashWithDirection;
        public Vector2 slashDir = Vector2.zero;
        public float distanceCalculated;
        public float distanceSlashed;
        public bool force;
        public float gravityEnabledFrames;
    }
    private SlashState _slashing = new SlashState();
    ...
    private void Start()
    {
        ...
        metrics = scoreKeeper.GetComponent<MetricManager>();
        ...
        _collisionMask = staticEnvLayerMask | movingPlatformLayerMask | nonStickLayerMask;
        _stickMask = staticEnvLayerMask | movingPlatformLayerMask;
    }
    ...
    private void UpdateTimers()
    {
        ...
        _slashing.gravityEnabledFrames--;
        _slashing.slashingFrames--;
    ...
    private void ReadjustTimers(float multiplier)
    {
        ...
        _slashing.gravityEnabledFrames = Mathf.RoundToInt(_slashing.gravityEnabledFrames * multiplier);
        _slashing.slashingFrames = Mathf.RoundToInt(_slashing.slashingFrames * multiplier);
    ...
    private float MoveMotor()
    {
        ...
        if (IsSlashing())
        {
            float normalizedTime = (float)(GetFrameCount(slashDuration) - _slashing.slashingFrames) /
                GetFrameCount(slashDuration);

            float distance = _slashFunction(0, slashDistance, normalizedTime);

            _velocity = _slashing.slashDir * GetSlashSpeed();
            MovePosition(_collider2D.bounds.center + (Vector3)_slashing.slashDir * (distance - _slashing.distanceCalculated));
            _slashing.distanceCalculated = distance;
        ...
        if(SceneManager.GetActiveScene().name != "Nirvana") //METRICS! =D
        {
            float distance = (curPos - targetPos).magnitude;
    ...
    private void UpdateState(bool forceSurroundingsCheck)
    {
        ...
        // since this is in updatedState we can end slashing if the timer is at 0.
        if(IsSlashing() && _slashing.slashingFrames <= 0)
        {
            EndSlash();
        }
        ...
        {
            //still slashing, bail
            _slashing.distanceSlashed += (_collider2D.bounds.center - _previousLoc).magnitude;
            return;
        }
        ...
        if (IsGrounded())
        {
            // ZACH: adding in falling through platforms logic
            if (dropThroughPlatform)
            {
                //test if we are on a droppable platform
                RaycastHit2D closestHit = GetClosestHit(_collider2D.bounds.center, Vector3.down, 0.4f); //dist is arbitrary
                GameObject platform = closestHit.collider.gameObject; 
                if(platform.GetComponent<PlatformEffector2D>() != null) //we are on droppable platform
                {
            ...
            //if touches ground, enable slashing
            _slashing.enabled = true;
    ...
    private bool UpdateMovingPlatform()
    {
        ...
        if (!IsDashing() && !IsSlashing() && _movingPlatformState.isOnPlatform)
        {
    ...
    private void CheckWallInteractionValidity()
    {
        ...
        _isValidWallInteraction = Physics2D.OverlapArea(min, max, _stickMask) != null; //ZACH
    }
    ...
    private void SetSlashFunctions()
    {
        _slashFunction = EasingFunctions.GetEasingFunction(slashEasingFunction);
        _slashDerivativeFunction = EasingFunctions.GetEasingFunctionDerivative(slashEasingFunction);
        _currentSlashEasingFunction = slashEasingFunction;
    }
    ...
    private void HandlePreJumping()
    {
        ...
        if (_jumping.pressed)
        {
            ...
            else if (_isValidWallInteraction && (enableWallJumps &&
            ...
            else if (_jumping.numAirJumps < numOfAirJumps && score.canDoubleJump)
            {
                _velocity.y = CalculateSpeedNeeded(_jumping.height);
    ...
    private void HandleFalling()
    {
        ...
        if (IsInAir() && !_ignoreGravity)
        {
            ...
            {
                //if (_dashing.gravityEnabledFrames < 0 && _slashing.gravityEnabledFrames < 0)
    ...
    {
        // set facing now and it won't be set again during slash
        SetFacing();

        if(!_slashing.slashWithDirection)
        {
            // we slash depending on our direction
            _slashing.slashDir = facingLeft ? Vector2.left : Vector2.right;
        }

        HitWords(slashDistance);

        _slashing.distanceSlashed = 0;
        _slashing.distanceCalculated = 0;
        _previousLoc = _collider2D.bounds.center;

        // play sound effect
        audio.clip = slashClip;
        audio.Play();

        //this will begin the slash this frame
        _slashing.slashingFrames = GetFrameCount(slashDuration) - 1;
        _slashing.force = false;

        ChangeState(MotorState.Slashing);
    }

    private void HitWords(float dist)
    {
        // raycast forward, check if hit anything
        Vector2 direction = facingLeft ? Vector2.left : Vector2.right;
        int wordMask = LayerMask.GetMask("Words");
        RaycastHit2D wordHit = Physics2D.Raycast(_collider2D.bounds.center, direction, dist, wordMask);
        if (wordHit && wordHit.collider != null)
        {
            wordHit.collider.gameObject.GetComponent<WordController>().Damage(25); //knock back word
            _hitWords = true;
            _slashing.enabled = true;
        }
        else
    ...
    private float GetSlashSpeed()
    {
        float normalizedTime = (float)(GetFrameCount(slashDuration) - _slashing.slashingFrames) /
            GetFrameCount(slashDuration);

        float speed = _slashDerivativeFunction(0, slashDistance, normalizedTime) / slashDuration;
